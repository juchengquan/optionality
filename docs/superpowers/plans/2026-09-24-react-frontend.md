...
- [ ] 0.1 Enrich `build_entries` so every entry carries `dte`, `fill`, the Position link
...
- [ ] 0.2 Move `_fill_pct` and the `days_to_expiry` call out of `routes/ui.py` into the domain
      modules. Proximity to a threshold is domain logic, not presentation.
- [ ] 0.3 Add `scope` and the Position link to `_to_dict` in `routes/monitors.py`. Today
      `/monitors` cannot say which Position a Monitor watches.
- [ ] 0.4 `routes/ui.py` keeps only formatting and column logic. **Acceptance: `_quote_rows`
      contains no arithmetic** — `_fmt_at` calls and the fill/bell column chain, nothing else.
- [ ] 0.5 A parity test: every field the dashboard renders must be reachable over HTTP. This is
      the test whose absence let the seam rot.

## Phase 1 — Build plumbing

- [ ] 1.1 `frontend/` with Vite + React + TypeScript. `make build-ui` emits to
      `src/optionality/service/static/app/` with `manifest: true`.
- [ ] 1.2 A catch-all asset route `GET /static/app/{path:path}` returning `FileResponse`, with
      a guard against path traversal. Not a mount, for the reason above.
- [ ] 1.3 `GET /app` renders a Jinja shell that reads Vite's `manifest.json` to find the hashed
...
- [ ] 1.4 No client-side router. It is one page; a router would reintroduce the base-path
      problem for no benefit.
- [ ] 1.5 Commit the built output. Add `make check-ui`, which rebuilds and fails if the result
...
- [ ] 2.1 Fetch `/quotes`, `/positions/values` and `/monitors`; poll at
      `MONITOR_INTERVAL_SECONDS`-aware intervals, matching the cookie the refresh selector sets.
- [ ] 2.2 Both tables with today's columns, `tabular-nums`, per-column precision, the fill bar,
...
- [ ] 2.3 Positions: entry, cost to close, P&L, exposure-signed greeks.
- [ ] 2.4 Muted groups with the retention countdown; health strip; "fetched at … · sweep every
      Ns".
- [ ] 2.5 Column pickers, persisting to the **same cookie names** (`ui_cols_single`,
...
- [ ] 3.1 Threshold, mute/unmute, delete, rename.
- [ ] 3.2 Create monitor and create combo.
- [ ] 3.3 Position entry via `PATCH /positions/{id}`.
- [ ] 3.4 **Missing endpoint:** the derive-a-wing-from-a-total flow exists only as
      `POST /ui/monitors/{id}/entry` returning HTML. Add a JSON equivalent.
- [ ] 3.5 Optimistic updates where safe; never for anything that changes an alarm threshold,
      where the server's answer is the only truth.

## Phase 4 — Tests

- [ ] 4.1 Vitest + Testing Library for components.
- [ ] 4.2 **Fixtures must mirror the real watchlist** — a spanning monitor, a wing rule, a leg
...
- [ ] 4.3 A rendering test for the fill/bell fallback chain under hidden columns.
- [ ] 4.4 Keep the Python suite as the contract test for the API.
