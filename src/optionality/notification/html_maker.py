...
def _gen_summary_html(summary: dict | pd.DataFrame):
    ...
    table_html = _df.to_html(index=False, escape=False)
...
def _gen_details_html(dt_details):
    ...
    for strike_date, _dfs in dt_details.items():
        ...
        for _df in _dfs.values():
            _details = pd.DataFrame(_df).to_html(index=False, escape=False).replace("\n", "")
